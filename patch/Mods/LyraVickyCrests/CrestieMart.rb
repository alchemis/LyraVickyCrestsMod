ModCacheInjection.hook(:marts) {
  $cache.marts[:LVCShop] = MartData.new(:LVCShop,
  {
    ...
    :Inventory => [
      ...
      ItemStock.of(:LVCTROPCREST).costs(80000).limit().properties(conditions: ['!searchItem(self.item) && $Trainer.numbadges >= 8']),
      ...
      ItemStock.of(:LVCBRONZCREST).costs(140000).limit().properties(conditions: ['!searchItem(self.item) && $Trainer.numbadges >= 14']),
